...
export default function AdminReports() {
  ...
  return (
    <div>
      ...
      {!data && <p style={{color:"#555"}}>Loading...</p>}
      {data && (
        <>
          {/* Summary cards */}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:"1rem",marginBottom:"2rem"}}>
            {[
              {label:"Total Bookings",val:data.total_bookings,color:gold},
              {label:"Completed",val:data.completed,color:"#2e7d32"},
              {label:"Cancelled",val:data.cancelled,color:"#c62828"},
              {label:"Revenue",val:`${data.revenue} SAR`,color:gold},
              ...
              <div key={c.label} style={{background:"#141414",border:"1px solid #222",borderRadius:10,padding:"1.25rem"}}>
                <div style={{color:"#555",fontSize:"0.73rem",fontWeight:600,textTransform:"uppercase",marginBottom:6}}>{c.label}</div>
          ...
          {/* By vehicle */}
          {data.by_vehicle && (
            <div style={{background:"#141414",border:"1px solid #222",borderRadius:10,padding:"1.5rem",marginBottom:"1.25rem"}}>
              <h3 style={{color:gold,fontSize:"0.9rem",marginBottom:"1rem"}}>Bookings by Vehicle Type</h3>
          ...
          {data.by_status && (
            <div style={{background:"#141414",border:"1px solid #222",borderRadius:10,padding:"1.5rem"}}>
              <h3 style={{color:gold,fontSize:"0.9rem",marginBottom:"1rem"}}>Bookings by Status</h3>
              <div style={{display:"flex",flexDirection:"column",gap:"0.5rem"}}>

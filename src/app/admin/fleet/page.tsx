...
export default function AdminFleet() {
  ...
  return (
    <div>
      <h1 style={{color:"#fff",fontSize:"1.2rem",fontWeight:800,marginBottom:"1.5rem"}}>Fleet Management {msg&&<span style={{color:gold,fontSize:"0.85rem",marginLeft:10}}>{msg}</span>}</h1>
          ...
          <tbody>
            {fleet.map(v=>(
              <tr key={v.id} style={{borderBottom:"1px solid #1a1a1a"}}>
                <td style={{padding:"0.6rem 1rem",color:"#fff",fontWeight:600}}>{v.name}</td>
                ...
                <td style={{padding:"0.6rem 1rem"}}><span style={{background:statusColor[v.status]??"#333",color:"#fff",padding:"2px 8px",borderRadius:4,fontSize:"0.72rem",fontWeight:700}}>{v.status?.toUpperCase()}</span></td>

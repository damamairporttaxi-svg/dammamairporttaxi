...
export default function AdminFleet() {
  ...
  return (
    <div>
      <h1 style={{color: "var(--text-primary)",fontSize:"1.2rem",fontWeight:800,marginBottom:"1.5rem"}}>Fleet Management {msg&&<span style={{color:"#0C58D1",fontSize:"0.85rem",marginLeft:10}}>{msg}</span>}</h1>

      <div style={{background:"#ffffff",border:"1px solid #e5e7eb",borderRadius:10,padding:"1.5rem",marginBottom:"1.5rem"}}>
        <h3 style={{color:"#0C58D1",fontSize:"0.9rem",marginBottom:"1rem"}}>{editing?"Edit Vehicle":"Add Vehicle"}</h3>
          ...
          <tbody>
            {fleet.map(v=>(
              <tr key={v.id} style={{borderBottom:"1px solid #1a1a1a"}}>
                ...
                <td style={{padding:"0.6rem 1rem",color:"#64748B",textTransform:"uppercase"}}>{v.vehicle_type}</td>
                <td style={{padding:"0.6rem 1rem",color:"#0C58D1",fontFamily:"monospace"}}>{v.plate}</td>
                <td style={{padding:"0.6rem 1rem",color:"#64748B"}}>{v.driver_name}</td>
                <td style={{padding:"0.6rem 1rem",color:"#64748B"}}>{v.driver_phone}</td>
            ...
            {fleet.length===0&&<tr><td colSpan={8} style={{padding:"2rem",textAlign:"center",color:"#64748B"}}>No vehicles yet</td></tr>}
          </tbody>
        </table>
      </div>
    </div>

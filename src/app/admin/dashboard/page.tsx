...
export default function AdminDashboard() {
  ...
  const gold = "#0C58D1";
  const cards = stats ? [
    { label: "Total Bookings", val: stats.total, color:"#0C58D1" },
    ...
    { label: "Total Revenue", val: `${stats.revenue} SAR`, color:"#0C58D1" },
  ] : [];
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
        ...
        <Link href="/admin/bookings" style={{ background:"#0C58D1", color: "#000", padding: "0.6rem 1.2rem", borderRadius: 6, textDecoration: "none", fontWeight: 700, fontSize: "0.85rem" }}>
          + New Booking
        </Link>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(170px,1fr))", gap: "1rem", marginBottom: "2rem" }}>
        {cards.map(c => (
          <div key={c.label} style={{ background:"#ffffff", border:"1px solid #e5e7eb", borderRadius: 10, padding: "1.25rem" }}>
            <div style={{ color:"#64748B", fontSize: "0.75rem", fontWeight: 600, textTransform: "uppercase", marginBottom: 6 }}>{c.label}</div>
            <div style={{ color: c.color, fontSize: "1.6rem", fontWeight: 800 }}>{c.val}</div>
          </div>
        ))}
        {!stats && <p style={{ color:"#64748B", gridColumn: "1/-1" }}>Loading stats...</p>}
      </div>
            ...
            <tbody>
              {recent.map((b: any) => (
                <tr key={b.id} style={{ borderBottom: "1px solid #1a1a1a" }}>
                  <td style={{ padding: "0.6rem 1rem", color:"#0C58D1", fontFamily: "monospace" }}>{b.ref}</td>
                  <td style={{ padding: "0.6rem 1rem", color: "var(--text-primary)" }}>{b.name}</td>
                  <td style={{ padding: "0.6rem 1rem", color:"#64748B", maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.pickup_location}</td>
                  <td style={{ padding: "0.6rem 1rem", color:"#64748B" }}>{b.pickup_date}</td>
                  <td style={{ padding: "0.6rem 1rem", color:"#64748B", textTransform: "uppercase" }}>{b.vehicle_type}</td>
                  <td style={{ padding: "0.6rem 1rem", color:"#0C58D1", fontWeight: 700 }}>{b.final_price} SAR</td>
                  ...
                  <td style={{ padding: "0.6rem 1rem" }}>
                    <Link href={`/admin/bookings/${b.id}`} style={{ color:"#0C58D1", textDecoration: "none", fontSize: "0.8rem" }}>Edit →</Link>
                  </td>
                </tr>
              ))}
              {recent.length === 0 && (
                <tr><td colSpan={8} style={{ padding: "1.5rem", textAlign: "center", color:"#64748B" }}>No bookings yet</td></tr>
              )}
            </tbody>
          </table>
        </div>

...
export default function AdminReviews() {
  ...
  return (
    <div>
      <h1 style={{color: "var(--text-primary)",fontSize:"1.2rem",fontWeight:800,marginBottom:"1.5rem"}}>Customer Reviews {msg&&<span style={{color:"#0C58D1",fontSize:"0.85rem",marginLeft:10}}>{msg}</span>}</h1>
      <div style={{display:"flex",flexDirection:"column",gap:"0.85rem"}}>
        {reviews.length===0&&<div style={{background:"#ffffff",border:"1px solid #e5e7eb",borderRadius:10,padding:"2rem",textAlign:"center",color:"#64748B"}}>No reviews yet</div>}
        {reviews.map(r=>(
          <div key={r.id} style={{background:"#ffffff",border:"1px solid #e5e7eb",borderRadius:10,padding:"1.25rem",display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"1rem"}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:"0.75rem",marginBottom:"0.5rem"}}>
                <span style={{color: "var(--text-primary)",fontWeight:700,fontSize:"0.9rem"}}>{r.customer_name}</span>
                <span style={{color:"#0C58D1",fontSize:"0.85rem"}}>{stars(r.rating)}</span>
                <span style={{color:"#64748B",fontSize:"0.75rem"}}>{r.created_at?.split("T")[0]}</span>
              ...
              <p style={{color:"#64748B",fontSize:"0.85rem",lineHeight:1.6,margin:0}}>{r.comment}</p>
            </div>

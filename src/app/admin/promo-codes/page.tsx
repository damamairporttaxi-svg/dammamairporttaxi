...
export default function AdminPromoCodes() {
  ...
  const gold = "#0C58D1";
  const inp = { background:"#ffffff", border:"1px solid #e5e7eb", borderRadius: 6, padding: "0.65rem 0.85rem", color: "var(--text-primary)", fontSize: "0.85rem" };

  return (
    <div>
      <h1 style={{ color: "var(--text-primary)", fontSize: "1.2rem", fontWeight: 800, marginBottom: "1.5rem" }}>Promo Codes {msg && <span style={{ color:"#0C58D1", fontSize: "0.85rem", marginLeft: 10 }}>{msg}</span>}</h1>

      {/* Create form */}
      <div style={{ background:"#ffffff", border:"1px solid #e5e7eb", borderRadius: 10, padding: "1.5rem", marginBottom: "1.5rem" }}>
        <h3 style={{ color:"#0C58D1", fontSize: "0.9rem", marginBottom: "1rem" }}>Create New Code</h3>
        ...
        <button onClick={create} style={{ background:"#0C58D1", color: "#000", border: "none", borderRadius: 6, padding: "0.65rem 1.5rem", fontWeight: 700, cursor: "pointer" }}>Create Code</button>
      </div>
            ...
            <tbody>
              {codes.map(c => (
                <tr key={c.id} style={{ borderBottom: "1px solid #1a1a1a" }}>
                  <td style={{ padding: "0.6rem 1rem", color:"#0C58D1", fontFamily: "monospace", fontWeight: 700 }}>{c.code}</td>
                  <td style={{ padding: "0.6rem 1rem", color:"#64748B" }}>{c.discount_type}</td>
                  <td style={{ padding: "0.6rem 1rem", color: "var(--text-primary)", fontWeight: 600 }}>{c.discount_value}{c.discount_type === "percentage" ? "%" : " SAR"}</td>
                  <td style={{ padding: "0.6rem 1rem", color:"#64748B" }}>{c.min_order ?? "-"}</td>
                  <td style={{ padding: "0.6rem 1rem", color:"#64748B" }}>{c.used_count}/{c.max_uses ?? "∞"}</td>
                  <td style={{ padding: "0.6rem 1rem", color:"#64748B" }}>{c.expires_at?.split("T")[0] ?? "Never"}</td>
              ...
              {codes.length === 0 && <tr><td colSpan={8} style={{ padding: "2rem", textAlign: "center", color:"#64748B" }}>No promo codes yet</td></tr>}
            </tbody>
          </table>
        </div>
      </div>

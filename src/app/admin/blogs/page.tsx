        ...
        <div style={{display:"flex",flexDirection:"column",gap:"0.85rem"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.75rem"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:"0.5rem",paddingTop:"1.2rem"}}>
              ...
              <label htmlFor="pub" style={{color:"#aaa",fontSize:"0.85rem"}}>Published</label>
            </div>
          </div>
          ...
          <div style={{display:"flex",gap:"0.75rem"}}>
            <button onClick={save} style={{background:gold,color:"#000",border:"none",borderRadius:6,padding:"0.65rem 1.5rem",fontWeight:700,cursor:"pointer"}}>{editing?"Update":"Publish"}</button>
            {editing&&<button onClick={()=>{setEditing(null);setForm({title:"",excerpt:"",content:"",category:"GCC Travel Guides",published:false});}} style={{background:"#222",color:"#aaa",border:"1px solid #333",borderRadius:6,padding:"0.65rem 1rem",cursor:"pointer"}}>Cancel</button>}
          </div>
        </div>
      </div>
      ...
      <div style={{background:"#141414",border:"1px solid #222",borderRadius:10,overflow:"hidden"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:"0.82rem"}}>
          <thead><tr style={{borderBottom:"1px solid #1e1e1e"}}>
            {["Title","Category","Status","Date",""].map(h=><th key={h} style={{padding:"0.65rem 1rem",color:"#555",textAlign:"left",fontWeight:600}}>{h}</th>)}
          </tr></thead>
          <tbody>
            {blogs.map(b=>(
              <tr key={b.id} style={{borderBottom:"1px solid #1a1a1a"}}>
                ...
                <td style={{padding:"0.6rem 1rem"}}>
                  ...
                  <button onClick={()=>toggle(b.id,b.published)} style={{background:"#1a3a1a",color:"#4caf50",border:"none",borderRadius:4,padding:"3px 8px",cursor:"pointer",fontSize:"0.75rem",marginRight:6}}>{b.published?"Unpublish":"Publish"}</button>
                  <button onClick={()=>del(b.id)} style={{background:"#3d0000",color:"#e53e3e",border:"none",borderRadius:4,padding:"3px 8px",cursor:"pointer",fontSize:"0.75rem"}}>Del</button>
                </td>
              </tr>
            ))}
            {blogs.length===0&&<tr><td colSpan={5} style={{padding:"2rem",textAlign:"center",color:"#555"}}>No blog posts yet</td></tr>}
          </tbody>
        </table>
      </div>
    </div>

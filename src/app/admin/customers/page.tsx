  ...
  return (
    <div>
      ...
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, email, phone..."
        style={{ background:"#ffffff", border:"1px solid #e5e7eb", borderRadius: 6, padding: "0.65rem 0.9rem", color: "var(--text-primary)", fontSize: "0.85rem", width: "100%", boxSizing: "border-box", marginBottom: "1rem" }} />
            ...
            <tbody>
              {filtered.map(c => (
                <tr key={c.id} style={{ borderBottom: "1px solid #1a1a1a" }}>
                  ...
                  <td style={{ padding: "0.6rem 1rem", color:"#64748B" }}>{c.email}</td>
                  <td style={{ padding: "0.6rem 1rem", color:"#64748B" }}>{c.phone ?? "-"}</td>
                  <td style={{ padding: "0.6rem 1rem", color: "var(--text-primary)", textAlign: "center" }}>{c.total_bookings}</td>
                  <td style={{ padding: "0.6rem 1rem", color:"#0C58D1", fontWeight: 700 }}>{c.total_spent} SAR</td>
                  <td style={{ padding: "0.6rem 1rem", color:"#64748B" }}>{c.created_at?.split("T")[0]}</td>
                </tr>
              ))}
              {filtered.length === 0 && <tr><td colSpan={6} style={{ padding: "2rem", textAlign: "center", color:"#64748B" }}>No customers found</td></tr>}
            </tbody>
          </table>
        </div>
      </div>

...
export default function AdminCustomers() {
  ...
  return (
    <div>
      <h1 style={{ color: "#fff", fontSize: "1.2rem", fontWeight: 800, marginBottom: "1.25rem" }}>Customers ({customers.length})</h1>
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, email, phone..."
        style={{ background: "#0c0c0c", border: "1px solid #333", borderRadius: 6, padding: "0.65rem 0.9rem", color: "#fff", fontSize: "0.85rem", width: "100%", boxSizing: "border-box", marginBottom: "1rem" }} />
            ...
            <tbody>
              {filtered.map(c => (
                <tr key={c.id} style={{ borderBottom: "1px solid #1a1a1a" }}>
                  <td style={{ padding: "0.6rem 1rem", color: "#fff", fontWeight: 600 }}>{c.name}</td>
                  <td style={{ padding: "0.6rem 1rem", color: "#aaa" }}>{c.email}</td>
                  <td style={{ padding: "0.6rem 1rem", color: "#aaa" }}>{c.phone ?? "-"}</td>
                  <td style={{ padding: "0.6rem 1rem", color: "#fff", textAlign: "center" }}>{c.total_bookings}</td>

...
export default function AdminBookings() {
  ...
  const gold = "#0C58D1";
  const input = { background:"#ffffff", border:"1px solid #e5e7eb", borderRadius: 6, padding: "0.6rem 0.9rem", color: "var(--text-primary)", fontSize: "0.85rem" };

  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem", gap: "1rem", flexWrap: "wrap" }}>
        <h1 style={{ color: "var(--text-primary)", fontSize: "1.2rem", fontWeight: 800, margin: 0 }}>All Bookings</h1>
        <Link href="/booking" target="_blank" style={{ background:"#0C58D1", color: "#000", padding: "0.6rem 1.2rem", borderRadius: 6, textDecoration: "none", fontWeight: 700, fontSize: "0.85rem" }}>
          + New Booking
        </Link>
      </div>
      ...
      <div style={{ background:"#ffffff", border:"1px solid #e5e7eb", borderRadius: 10, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82rem" }}>
            ...
            <tbody>
              {loading && <tr><td colSpan={11} style={{ padding: "2rem", textAlign: "center", color:"#64748B" }}>Loading...</td></tr>}
              {!loading && filtered.length === 0 && <tr><td colSpan={11} style={{ padding: "2rem", textAlign: "center", color:"#64748B" }}>No bookings found</td></tr>}
              {filtered.map(b => (
                <tr key={b.id} style={{ borderBottom: "1px solid #1a1a1a" }}>
                  <td style={{ padding: "0.6rem 0.9rem", color:"#0C58D1", fontFamily: "monospace", whiteSpace: "nowrap" }}>{b.ref}</td>
                  <td style={{ padding: "0.6rem 0.9rem", color: "var(--text-primary)", whiteSpace: "nowrap" }}>{b.name}</td>
                  <td style={{ padding: "0.6rem 0.9rem", color:"#64748B" }}>{b.phone}</td>
                  <td style={{ padding: "0.6rem 0.9rem", color:"#64748B", maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.pickup_location}</td>
                  <td style={{ padding: "0.6rem 0.9rem", color:"#64748B", maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.dropoff_location}</td>
                  <td style={{ padding: "0.6rem 0.9rem", color:"#64748B", whiteSpace: "nowrap" }}>{b.pickup_date}</td>
                  <td style={{ padding: "0.6rem 0.9rem", color:"#64748B" }}>{b.pickup_time}</td>
                  <td style={{ padding: "0.6rem 0.9rem", color:"#64748B", textTransform: "uppercase" }}>{b.vehicle_type}</td>
                  <td style={{ padding: "0.6rem 0.9rem", color:"#0C58D1", fontWeight: 700, whiteSpace: "nowrap" }}>{b.final_price} SAR</td>
                  ...
                  <td style={{ padding: "0.6rem 0.9rem", whiteSpace: "nowrap" }}>
                    <Link href={`/admin/bookings/${b.id}`} style={{ color:"#0C58D1", textDecoration: "none", fontSize: "0.8rem", marginRight: 8 }}>Edit</Link>
      ...
      <p style={{ color:"#64748B", fontSize: "0.78rem", marginTop: "0.75rem" }}>{filtered.length} booking(s) shown</p>
    </div>
  );
}

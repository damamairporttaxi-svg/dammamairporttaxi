  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto" }}>
      <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1.5rem", flexWrap: "wrap" }}>
        <button onClick={() => window.print()} style={{ background: gold, color: "#000", border: "none", borderRadius: 6, padding: "0.65rem 1.25rem", fontWeight: 700, cursor: "pointer", fontSize: "0.85rem" }}>
          Print / Save PDF
        </button>
        <button onClick={() => fetch("/api/send-invoice-email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ booking_id: id }) }).then(() => alert("Invoice emailed!"))}
          style={{ background: "#222", color: "#aaa", border: "1px solid #333", borderRadius: 6, padding: "0.65rem 1.25rem", cursor: "pointer", fontSize: "0.85rem" }}>
          Email Invoice
        </button>
      </div>
      ...
      <div id="invoice" style={{ background: "#fff", color: "#111", padding: "2.5rem", borderRadius: 10, fontFamily: "Arial, sans-serif" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "2rem", borderBottom: "2px solid #f5c518", paddingBottom: "1.5rem" }}>
          <div>
            ...
            <p style={{ margin: "4px 0 0", color: "#888", fontSize: "0.85rem" }}>Dammam Airport Taxi</p>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: "0.75rem", color: "#888" }}>Invoice #</div>
            <div style={{ fontWeight: 700, fontFamily: "monospace" }}>{b.ref}</div>
            <div style={{ fontSize: "0.75rem", color: "#888", marginTop: 8 }}>Date</div>
          ...
          <tbody>
            {rows.map(([l, v]) => (
              <tr key={l} style={{ borderBottom: "1px solid #f0f0f0" }}>
                <td style={{ padding: "0.5rem 0", color: "#888", width: "40%" }}>{l}</td>
        ...
        <div style={{ background: "#f9f9f9", borderRadius: 8, padding: "1.25rem", marginBottom: "2rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            <span style={{ color: "#888" }}>Base Fare</span><span>{b.price} SAR</span>
          </div>
          {b.discount_amount > 0 && (
          ...
          <div style={{ display: "flex", justifyContent: "space-between", borderTop: "2px solid #eee", paddingTop: 8, fontWeight: 800, fontSize: "1.1rem" }}>
            <span>Total Due</span><span style={{ color: "#f5c518" }}>{b.final_price} SAR</span>
          </div>
        </div>

        <div style={{ textAlign: "center", color: "#888", fontSize: "0.78rem" }}>

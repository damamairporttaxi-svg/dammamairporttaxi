    ...
    <div style={{ maxWidth: 500, margin: "0 auto" }}>
      <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1.5rem" }}>
        ...
        <button onClick={() => fetch("/api/send-receipt-email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ booking_id: id }) }).then(() => alert("Receipt emailed!"))}
          style={{ background: "#222", color: "#aaa", border: "1px solid #333", borderRadius: 6, padding: "0.65rem 1.25rem", cursor: "pointer", fontSize: "0.85rem" }}>Email Receipt</button>
      </div>
      <div style={{ background: "#fff", color: "#111", padding: "2rem", borderRadius: 10, textAlign: "center", fontFamily: "Arial, sans-serif" }}>
        ...
        <p style={{ color: "#888", fontSize: "0.82rem", margin: "0 0 1.5rem" }}>Dammam Airport Taxi</p>
        <div style={{ fontSize: "2rem", fontWeight: 800, color: "#f5c518", marginBottom: "1.5rem" }}>
          {b.final_price} SAR
        </div>
        <div style={{ textAlign: "left", marginBottom: "1.5rem" }}>
          {[["Ref",b.ref],["Name",b.name],["Route",`${b.pickup_location} → ${b.dropoff_location}`],["Date",b.pickup_date],["Vehicle",b.vehicle_type?.toUpperCase()]].map(([l,v]) => (
            <div key={String(l)} style={{ display: "flex", justifyContent: "space-between", padding: "0.4rem 0", borderBottom: "1px solid #f0f0f0", fontSize: "0.85rem" }}>
              <span style={{ color: "#888" }}>{l}</span><span style={{ fontWeight: 600 }}>{String(v)}</span>
            </div>
          ))}
        </div>
        ...
        <p style={{ color: "#888", fontSize: "0.72rem", marginTop: "1.5rem" }}>+966 56 948 7569 · haramtaxiservice@gmail.com</p>
      </div>
    </div>
  );
}
